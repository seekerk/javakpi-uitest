...
public class Location extends BaseRDF {
    ...
    //------------ lat --------------
        ...
        public ArrayList<Double> lat() {
            ...
            return getDoubleInTriples(lat_URI);
        }
    ...
    //------------ lon --------------
        ...
        public ArrayList<Double> lon() {
            ...
            // search in triple store
            return getDoubleInTriples(lon_URI);
        }
        ...
        if (getStringInTriples(RDF_TYPE_URI).isEmpty()) {
            ...
            newTriples.add(createTriple(getID(), RDF_TYPE_URI, getURI()));
                ...
                if (_lat_new != null) {
                    ...
                    ArrayList<Double> oldVals = getDoubleInTriples(lat_URI);
                    ...
                    _lat_new.stream().forEach((Double val) -> {
                        newTriples.add(createTriple(getID(), lat_URI, val.toString(), "uri", "literal"));
                    });
                    oldVals.stream().forEach((val) -> {
                        removeTriples.add(createTriple(getID(), lat_URI, val.toString(), "uri", "literal"));
                    });
        	    _lat_new = null;
                }
        //-----------------------
                if (_lon_new != null) {
                    // получаем старые значения
                    ArrayList<Double> oldVals = getDoubleInTriples(lon_URI);
                    ...
                    _lon_new.stream().forEach((Double val) -> {
                        newTriples.add(createTriple(getID(), lon_URI, val.toString(), "uri", "literal"));
                    });
                    oldVals.stream().forEach((val) -> {
                        removeTriples.add(createTriple(getID(), lon_URI, val.toString(), "uri", "literal"));
                    });
        	    _lon_new = null;
                }
        //-----------------------
